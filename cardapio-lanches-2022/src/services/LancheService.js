...
const parseTransformItem = (response) => parseResponse(response).then(transformLanche);
...
/* criação de novo lanche cadastrado, enviando para o back-end */
    ...
    updtateById: (id, lanche) => fetch(Api.updateLancheById(id), { method: "PUT", body: JSON.stringify(lanche), mode: "cors", headers: {
        "Content-Type": "application/json",
        } }).then(parseResponse),
